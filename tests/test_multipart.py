...
class TestParseOptionsHeader(unittest.TestCase):
    ...
    def test_plain_filename_authoritative_over_extended(self) -> None:
        # A smuggled `filename*` must not override the plain `filename` an
        # upstream validator would have inspected.
        _, p = parse_options_header(b"form-data; name=\"f\"; filename=\"safe.txt\"; filename*=utf-8''evil.php")

        self.assertEqual(p, {b"name": b"f", b"filename": b"safe.txt"})

    def test_ignores_percent_encoded_bytes_in_extended_param(self) -> None:
        # The extended syntax is never percent-decoded, so traversal segments
        # and control bytes cannot be smuggled through it.
        _, p = parse_options_header(b"form-data; filename=\"safe.txt\"; filename*=utf-8''..%2Fevil%00.php")

        self.assertEqual(p, {b"filename": b"safe.txt"})
